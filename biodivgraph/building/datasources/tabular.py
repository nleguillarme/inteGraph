...
class TabularDataImport(Job):
    def __init__(self, cfg):
        ...
        self.output_file = os.path.join(self.triples_dir, self.internal_id + ".nq")
    ...
    def map(self, df):
        df = df.dropna()
        self.mapper.run_mapping(df, self.internal_id + ".{}.nt".format(self.nb))
        self.nb += 1
    ...
    def merge(self):
        ...
        triples_files = glob.glob(prefix + "*.nt")
        g = Graph()
        for file in triples_files:
            g.parse(file, format="nt")  # format="nquads")
    ...
    def run(self):
        self.create_tmp_dir()
        rule_file = (
            "/usr/local/airflow/dags/example/biodiv-graph/mappings/inter-to-globi.yml"
        )
        ...
        for chunk in df_reader:
            self.logger.info("Process chunk #{}".format(n))
            # chunk = chunk.head()
