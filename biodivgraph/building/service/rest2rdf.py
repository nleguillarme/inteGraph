...
class REST2RDF(Service):
    def __init__(self, config):
        ...
        self.config.output_data_dir = os.path.join(self.config.output_dir, "data")
        ...
        if "extractor" in self.config:
            components = self.config.extractor.split(".")
            module_name = ".".join(components[:-1])
            module = importlib.import_module(module_name)
            extractor_class = getattr(module, components[-1])
            self.extractor = extractor_class(self.config)
    ...
    def run(self):
        ...
        if self.extractor:
            self.extract_data(self.get_extracted_data_filename())
    ...
    def get_extracted_data_filename(self):
        return os.path.join(
            self.config.output_data_dir, self.get_id() + "_extracted.csv"
        )
